...
export const getUserTeachedClassRooms = action$ => action$
  ...
  .mergeMap(({headers, payload}) => Observable
    .ajax.get(`http://${serverConfig.host}:${serverConfig.port}/api/classroom/teached/${payload.user}`, headers)
    .map(res => res.response)
...
export const getUserFollowedClassRooms = action$ => action$
  ...
  .map(signRequest)
  .mergeMap(({headers, payload}) => Observable
    .ajax.get(`http://${serverConfig.host}:${serverConfig.port}/api/classroom/followed/${payload.user}`, headers)
    .map(res => res.response)
