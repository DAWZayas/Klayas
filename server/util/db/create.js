...
// docker run command
const cmd = `docker run -d -p 28015:28015 -p 8090:8080 -v ${dbPath}:/data --name klayasdb rethinkdb`;
...
start.stdout.on('data', (data) => {
  if (data) {
    console.log('Sucessfully created klayasdb\n');
  }
});
...
start.stderr.on('data', (data) => {
  if (data === "Unable to find image 'rethinkdb:latest' locally\n" || dbImage) {
    ...
    console.log('Error while creating klayasdb:', data);
  }
});
